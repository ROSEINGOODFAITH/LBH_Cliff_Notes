...
- Review card v2: handle links to the live profile, platform + display name shown, empty stats hidden, inline email add (`POST /api/pulse/creator`), and Tier A/B are disabled in UI + refused by the decision API (409) when no email AND no address exists — tiering an unreachable creator previously no-oped silently.
- SUPERSEDES §5.5 (2026-07-10, owner-approved): the address form is also a front door. The "Would you want to?" selection routes submissions — "Be removed" → churned opt-out (suppression record created even for unknowns; posted/paid keep their stage); known Invited/Replied + address + wants-PULSE-now → ship immediately (unchanged); EVERYTHING else (unknown people, earlier stages, "another scent"/"future" choices) → stage `review` with shipping + formChoices saved in rawModash — never raw `sourced`, never dropped. Creators with rawModash.shipping skip the Smartlead invite at tiering: outreach-on-tiered creates the discount code then draft-orders directly → `onboarded`. Fuzzy field matching on the webhook (address/street, city/town, state/province, zip/postal, would-you-want-to). Migration 0004 repairs previously mis-filed form rows (sourced + first_party + ig_handle) → review.
- Prospect CSV import captures ALL email addresses in a cell: first → creators.email, rest → rawModash.emails.
